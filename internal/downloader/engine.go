...
func (e *Engine) buildArgs(config models.DownloadConfig) []string {
	...
	if config.IsPlaylist { args = append(args, "--yes-playlist") } else { args = append(args, "--no-playlist") }

	if config.DownloadMode == "Audio" {
		args = append(args, "-x")
		switch config.Quality {
		case "mp3": args = append(args, "--audio-format", "mp3", "--audio-quality", "0")
		case "m4a": args = append(args, "--audio-format", "m4a")
		default: args = append(args, "--audio-format", "best")
		}
	} else {
		args = append(args, "--merge-output-format", "mp4")
		switch config.Quality {
		case "4k": args = append(args, "-f", "bestvideo[height<=2160]+bestaudio/best")
		case "1080p": args = append(args, "-f", "bestvideo[height<=1080]+bestaudio/best")
		case "720p": args = append(args, "-f", "bestvideo[height<=720]+bestaudio/best")
		default: args = append(args, "-f", "bestvideo+bestaudio/best")
		}
	}

	if config.TrimStart != "" {
		section := fmt.Sprintf("*%s-%s", config.TrimStart, config.TrimEnd)
		if config.TrimEnd == "" { section = fmt.Sprintf("*%s-inf", config.TrimStart) }
		args = append(args, "--download-sections", section, "--force-keyframes-at-cuts")
	}
	if config.UseSponsorBlock { args = append(args, "--sponsorblock-remove", "all") }
	...
	if config.CookiesPath != "" { args = append(args, "--cookies", config.CookiesPath) }
	return args
}
...
func (e *Engine) executeCommand(args []string, callback func(models.ProgressUpdate)) error {
	...
	for errScanner.Scan() { errOutput += errScanner.Text() + "\n" }
	if err := cmd.Wait(); err != nil {
		if errOutput != "" { return fmt.Errorf("%v | %s", err, errOutput) }
		return err
	}
	return nil
}
